 ...
 * @class MeeyDirections
 *
 * @param {Object} options
 * @param {Array} [options.styles] Override default layer properties of the [directions source]
 * @param {String} [options.accessToken=null] Required unless `meeygl.accessToken` is set globally
 * @param {String} [options.api="https://api.mapbox.com/directions/v5/"] Override default routing endpoint url
 * @param {Boolean} [options.interactive=true] Enable/Disable mouse or touch interactivity from the plugin
 * @param {String} [options.profile="car"] Routing profile to use. Options: `car`, `moto`, `foot`
 ...
 * @param {Object} [options.geocoder] Accepts an object containing the query parameters as [documented here]
 ...
 * @param {String} [options.language="en"] The language of returned turn-by-turn text instructions. See supported languages
 ...
 * @param {String} [options.exclude=null] Exclude certain road types from routing. The default is to not exclude anything. Search for `exclude` in `optional parameters`
 * @param {number | PaddingOptions} [options.routePadding=80] Specify padding surrounding route. A single number of pixels or a [PaddingOptions] object.
 * @example
 ...
 *   profile: 'car'
 * });
 * // add to your Meeygl map
 * map.addControl(directions);
 *
 * @return {MeeyDirections} `this`
 */
export default class MeeyDirections {
  ...
  /**
   * Sets destination. _Note:_ calling this method requires the [map load event]
   * to have run.
   * @param {Array<number>|String} query An array of coordinates [lng, lat] or location name as a string.
   * @returns {MeeyDirections} this
   */
   ...
   * method requires the [map load event]
   * to have run.
